...
def get_token_info(token):
    ...
    try:
        ...
        if token_info and 'metadata' in token_info and isinstance(token_info['metadata'], str):
            try:
...
def get_richlist(symbol):
    richlist = he_api.find("tokens", "balances", query={"symbol": symbol}, limit=100, indexes=[{"index": "balance", "descending": True}])
    return richlist
...
def market(token):
    ...
    return render_template('market.html', token=token, token_info=token_info, chart_json=chart_json, 
                          buy_book=buy_book, sell_book=sell_book, trade_history=trade_history)
...
def view(token):
    ...
    return render_template('view.html', token=token, token_info=token_info, richlist=richlist)
